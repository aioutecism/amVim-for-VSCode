...
export class ActionMoveCursor {
    ...
    static byMotions(args: {
        ...
        args.noEmptyAtLineEnd =
            args.noEmptyAtLineEnd === undefined ? false : args.noEmptyAtLineEnd;
        ...
        activeTextEditor.selections = activeTextEditor.selections.map(
            (selection, i) => {
                ...
                let active = args.motions.reduce(
                    (position, motion) => {
                        return motion.apply(position, {
                            preferredColumn:
                                ActionMoveCursor
                                    .preferredColumnBySelectionIndex[i],
                        });
                    },
                    args.isVisualMode
                ...
                if (args.isVisualMode) {
                    ...
                    const anchorLineLength = activeTextEditor.document.lineAt(
                        anchor.line,
                    ).text.length;
                    const activeLineLength = activeTextEditor.document.lineAt(
                        active.line,
                    ).text.length;

                    if (
                        active.isAfterOrEqual(anchor) &&
                        active.character < activeLineLength
                    ) {
                    ...
                    if (anchor.isBefore(active)) {
                        ...
                        active = active.with(
                            undefined,
                            activeTextEditor.document.lineAt(active.line).text
                                .length,
                        );
                    } else {
                        anchor = anchor.with(
                            undefined,
                            activeTextEditor.document.lineAt(anchor.line).text
                                .length,
                        );
                    ...
                    if (args.noEmptyAtLineEnd) {
                        const lineEndCharacter = document.lineAt(active.line)
                            .text.length;
                        if (
                            lineEndCharacter !== 0 &&
                            active.character === lineEndCharacter
                        ) {
                            active = active.translate(0, -1);
                        }
                    }
