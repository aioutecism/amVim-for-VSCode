      ...
      (selection, i) => {
        ...
        let active = args.motions.reduce((position, motion) => {
          return motion.apply(position, {
            preferredColumn:
              ActionMoveCursor.preferredColumnBySelectionIndex[i],
          });
        ...
        if (args.isVisualMode) {
          ...
          const anchorLineLength = activeTextEditor.document.lineAt(anchor.line)
            .text.length;
          const activeLineLength = activeTextEditor.document.lineAt(active.line)
            .text.length;
