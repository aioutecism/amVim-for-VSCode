...
export class ModeInsert extends Mode {
  ...
  private maps: CommandMap[] = [
    {
      keys: 'ctrl+w',
      actions: [
        () => ActionDelete.byMotions({ motions: [MotionWord.prevStart()] }),
      ],
    },
    {
      keys: 'ctrl+u',
      actions: [
        () => ActionDelete.byMotions({ motions: [MotionLine.firstNonBlank()] }),
      ],
    },

    {
      ...
      actions: [
        ...
        () =>
          ActionSelection.shrinkToActives().then(
            isShrunken =>
  ...
  private processRecord(): void {
    ...
    searchLimit =
      this.recordStartLineText.length - this.recordStartPosition.character + 1;
  ...
  onDidChangeTextEditorSelection(): void {
    ...
    if (
      this.textEditor.selection.active.line !== this.recordStartPosition.line
    ) {
      this.endRecord();
    }
  }
